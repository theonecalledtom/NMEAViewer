...
namespace NMEAViewer
{
    ...
    class TackAnalysis
    {
        ...
        List<int> FindTackPoints(int iStartSearch, int iEndSearch)
        {
            ...
            for (int i = iStartSearch; i < iToSearch - 1; i++)
            {
                ...
                if ((fAwa_0 > 0.0) ^ (fAwa_Plus1 > 0.0))
                {
                    ...
                    double fAngleAveragePre = Util.CalculateAngleAverage(m_Data, NMEACruncher.DataTypes.AWA, iStart, i);
                    double fAvPrePoint = Util.ContainAngleMinus180To180(fAngleAveragePre);

                    int iTail = System.Math.Min(i + 20, iToSearch);
                    double fAngleAveragePost = Util.CalculateAngleAverage(m_Data, NMEACruncher.DataTypes.AWA, i, iTail);
                    double fAvPostPoint = Util.ContainAngleMinus180To180(fAngleAveragePost);
        ...
        {
            return Util.CalculateAngleAverage(m_Data, NMEACruncher.DataTypes.AWA, iStart, iFinish);
        }
        ...
        TackAnalysisData RunTackAnalysis(int iFirstSample, int iLastSample, bool bUseAWARange, double fAWAMin, double fAWAMax)
        {
            ...
            rOutData.SetValue(TackAnalysisData.eTackDataTypes.InitialHeading,
                                Util.ContainAngle0To360(m_Data.GetDataAtIndex(iFirstSample, NMEACruncher.DataTypes.BoatHeading))
                    );
            ...
            double fDelta = Util.ShortAngle(m_Data.GetDataAtIndex(iStartOfTurn, NMEACruncher.DataTypes.AWA), m_Data.GetDataAtIndex(iThroughWind, NMEACruncher.DataTypes.AWA));
            ...
            while (iStartOfTurn > iFirstSample)	//? Could use zero and do the sample range automatically eventually
            {
                ...
                double fPriorDelta = Util.ShortAngle(fEarlierAWAAv, fLaterAWAAv);
            ...
            fDelta = Util.ShortAngle(m_Data.GetDataAtIndex(iStartOfTurn, NMEACruncher.DataTypes.AWA), m_Data.GetDataAtIndex(iThroughWind, NMEACruncher.DataTypes.AWA));
            while (iEndOfTurn < kMaxSearchForEnd)
            {
                ...
                if ((fPriorDelta > 0.0f) ^ (fDelta > 0.0f))
                {
                    ...
                    while (iEndOfTurn < kMaxSearchForEnd)	//? Could use zero and do the sample range automatically eventually
                    {
                        ...
                        double fNewPriorDelta = Util.ShortAngle(fEarlierAWAAv, fLaterAWAAv);
            ...
            if (bUseAWARange)
            {
                ...
                double fAWAEntry = Math.Abs(Util.ContainAngleMinus180To180(AvAWAInRange(iPreAngleStart, iStartOfTurn)));
                ...
                double fAWAExit = Math.Abs(Util.ContainAngleMinus180To180(AvAWAInRange(iEndOfTurn, iEndOfPostTurn)));
            ...
            rOutData.SetValue(TackAnalysisData.eTackDataTypes.AverageTWD, Util.ContainAngle0To360(Util.CalculateAngleAverage(m_Data, NMEACruncher.DataTypes.TWD, iStartOfTurn, iEndOfTurn)));
            rOutData.SetValue(TackAnalysisData.eTackDataTypes.AverageTWS, Util.CalculateAverage(m_Data, NMEACruncher.DataTypes.TWS, iStartOfTurn, iEndOfTurn));
            ...
            if (iNumSamplesInEntry > 0)
            {
                ...
                double fEntryDirection = Util.ContainAngle0To360(Util.CalculateAngleAverage(m_Data, NMEACruncher.DataTypes.GPSHeading, iFirstSample, iEntryEnd));
                double fExitDirection = Util.ContainAngle0To360(Util.CalculateAngleAverage(m_Data, NMEACruncher.DataTypes.GPSHeading, iExitStart, iLastSample));
                double fTackingAngle = Util.ShortAngle(fEntryDirection, fExitDirection);
                rOutData.SetValue(TackAnalysisData.eTackDataTypes.AngleTackedThrough, fTackingAngle);

                double fEntryDirection_Steered = Util.ContainAngle0To360(Util.CalculateAngleAverage(m_Data, NMEACruncher.DataTypes.BoatHeading, iFirstSample, iEntryEnd));
                double fExitDirection_Steered = Util.ContainAngle0To360(Util.CalculateAngleAverage(m_Data, NMEACruncher.DataTypes.BoatHeading, iExitStart, iLastSample));
                double fTackingAngle_Steered = Util.ShortAngle(fEntryDirection_Steered, fExitDirection_Steered);
                ...
                double fFinalHeading = Util.ContainAngle0To360(Util.CalculateAngleAverage(m_Data, NMEACruncher.DataTypes.GPSHeading, iBackToNinetyPercent, iEndOfFinalHeading));
                ...
                {
                    double fHeadingDelta = Util.ShortAngle(GPSHeading(i), fFinalHeading);
